  ...
  useEffect(() => {
    ...
    if (!hasProcessed) {
      ...
      try {
        ...
        await api.post('/api/integration/gmail/callback', { code });
        ...
        console.error('Error processing Gmail authorization:', error);
        setStatus('error');
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <div className={`max-w-md w-full p-6 rounded-lg shadow-lg ${isDarkMode ? 'bg-[#1a1a1a] text-white' : 'bg-white text-gray-900'}`}>
        <h1 className="text-xl font-semibold mb-4 text-center">Gmail Integration</h1>
