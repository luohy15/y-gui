...
export const McpServerFormModal: React.FC<McpServerFormModalProps> = ({
  ...
  // Determine initial server type based on initialServer or default to 'sse'
  const determineInitialServerType = (): ServerType => {
    if (initialServer) {
      return initialServer.command ? 'stdio' : 'sse';
    }
    return 'sse'; // Default to 'sse' for new servers
  };

  const [serverType, setServerType] = useState<ServerType>(determineInitialServerType());
  ...
  useEffect(() => {
    if (initialServer) {
      ...
      setServerType(initialServer.command ? 'stdio' : 'sse');
  ...
  const handleServerTypeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newType = e.target.value as ServerType;
    setServerType(newType);

    // Reset fields that aren't relevant for the new type
    if (newType === 'stdio') {
      setFormData(prev => ({
        ...prev,
        url: null,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (serverType === 'sse' && !formData.url) {
      setError('URL is required for sse server type');
      setIsSubmitting(false);
      return;
    }
                ...
                <div className="flex space-x-4 mt-1">
                  <label className={`inline-flex items-center ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    <input
                      ...
                      className={`form-radio h-4 w-4 ${isDarkMode ? 'text-blue-500' : 'text-blue-600'}`}
                    />
                    <span className="ml-2">SSE (Server-Sent Events)</span>
                  </label>
                  <label className={`inline-flex items-center ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    <input
                      type="radio"
                      ...
                      className={`form-radio h-4 w-4 ${isDarkMode ? 'text-blue-500' : 'text-blue-600'}`}
                    />
                    <span className="ml-2">STDIO (Command-based)</span>
                  </label>
                </div>
              </div>
